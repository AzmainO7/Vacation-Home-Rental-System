...
BEGIN
    ...
    FOR R IN (Select prp_id, po_id, prp_desc, prp_address, city, zip_code from PROPERTY where c_location = 'Chittagong') LOOP
    ...
    FOR R IN (Select * from BOOKING b LEFT JOIN PROPERTY_2 p ON b.prp_id = p.prp_id) LOOP
    ...
    FOR R IN (Select * from PropertyOwner po LEFT JOIN PROPERTY_2 p ON po.po_id = p.po_id) LOOP
    ...
    FOR R IN (Select * from REVIEW r LEFT JOIN PROPERTY_2 p ON r.prp_id = p.prp_id) LOOP
